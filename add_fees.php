...
<h3 class="text-primary">Add Student Fees</h3> </div>
...
<form class="form-horizontal" method="POST"  id="submitProductForm" action="php_action/create_fees.php" method="POST" enctype="multipart/form-data">
...
<?php 
                        $sql = "SELECT * FROM student,courses where student.course_id = courses.id";
                              ...
                              data-fees="<?php echo $res['total_amount']?>" >
                            <?php echo $res['name']; ?>
                            </option>
                                 <?php   
                                } // while
...
<div class="col-sm-9">
<input type="text" class="form-control" id="balanceamt"  name="balanceamount" autocomplete="off"  />
</div>
</div>
</div> -->
